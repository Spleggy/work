...
def draw(first, second, third):
  ...
  if ((perimeter.is_integer) and (valid == 1)):
    ...
    for count in range(perimeter):
      print(" " * (perimeter - count - 1) + "*" * ((2 * count) + 1), sep = "")
...
def run():
  ...
  print("What option do you want? 1) Validate 2) Identify 3) Calculate Perimeter 4) Calculate Area 5) Draw equilateral triangle with side of perimeter of given sides")
